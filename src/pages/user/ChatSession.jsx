...
function StatusBar({ stage, message, isStreaming }) {
  ...
  return (
    <div className="px-4 py-2 border-t border-border-default bg-background-subtle flex items-center gap-2 text-xs">
      ...
      <span className={`font-medium ${isDone ? 'text-success-600' : 'text-text-secondary'}`}>
        {label}
      </span>
      {!isDone && isStreaming && (
...
export default function ChatSession() {
  ...
  async function sendQuestion() {
    ...
    // Persist the user message first (ChatGPT-like: history is the source of truth).
    ...
    setMessages((prev) => [...prev, userMessage, { id: assistantLocalId, role: 'assistant', type: 'retriever', text: '' }]);
    setQuestion('');
    setIsStreaming(true);
    ...
    try {
      await streamRetrieverQuery(
        { query: text, top_k: 8, signal, files: filesToSend.length ? filesToSend : undefined },
        {
          ...
          onToken: (t) => {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === assistantLocalId ? { ...m, text: (m.text || '') + t } : m
              )
            );
          },
          onError: (msg) => {
      ...
      // Save final assistant message to history (only if we have content).
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // If invalid/not found, send back to home.
  ...
  // If the user navigates away (e.g. back button), abort any in-flight stream
  ...
  function bubbleClass(msg) {
    if (msg.role === 'user') {
      return 'border border-border-default bg-background-subtle';
    }
    if (msg.type === 'refusal') {
      return 'bg-error-soft border border-error-500/30';
    }
    return 'bg-background-surface border border-border-default';
  }

  return (
    <UserChatLayout title={conversationTitle} noPageScroll>
      <div className="h-full min-h-0 flex flex-col gap-4">
        <section className="bg-background-surface border border-border-default rounded-lg overflow-hidden flex-1 min-h-0 flex flex-col">
            ...
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`rounded-lg p-3 ${bubbleClass(msg)}`}>
                  ...
                  {/* Show status placeholder while the assistant message is empty during streaming */}
                  {isStreamingThis && !msg.text ? (
                    <span className="text-sm text-text-muted italic">
                      {retrieverStatus.message || 'Thinking…'}
                    </span>
                  ) : (
          ...
          {isStreaming && (
            <StatusBar
              stage={retrieverStatus.stage}
              message={retrieverStatus.message}
